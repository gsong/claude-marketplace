...
description: Use when the user wants to investigate and triage code review findings for a PR, or when the user invokes /gs:gh-tools:triage with a PR number. Requires running /gs:gh-tools:review first.
---
...
2. If the directory doesn't exist, stop with: "Error: Directory `ai-swap/pr-review-$ARGUMENTS/` not found. Run `/gs:gh-tools:review $ARGUMENTS` first."
