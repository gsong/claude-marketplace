...
if [[ $TOTAL_DOCS -gt 0 && $STUB_DOCS -gt 0 ]]; then
  ...
  if [[ $STUB_RATIO -ge 60 ]]; then
    touch "$COOLDOWN_FILE"
    echo "Docs exist but most need content.${LOCATIONS} Consider running ai-docs:update or editing docs manually before relying on lookups."
    exit 0
  fi
fi
...
echo "Before writing or modifying code, use Skill(\"ai-docs:lookup\", \"your question\") to check project conventions.${LOCATIONS} Skip for: conversation, trivial fixes, topics already looked up this session, or git/shell operations."
